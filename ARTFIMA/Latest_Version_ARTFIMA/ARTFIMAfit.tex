...
\Delta^{d,\lambda} f(t)=(I-e^{-\lambda }B)^d f(t)=\sum_{j=0}^\infty \omega^{d,\lambda}_{j}f(t-j)
\end{equation}
where $d>0$, $\rev{d\notin{\mathbb Z}}$, $\lambda>0$, $\rev{Bf(t) = f(t-1)}$ is the shift operator, and
...
\rev{The exact form of the tempered fractional difference operator \eqref{eq:TFdiffDef} comes from the emerging field of tempered fractional calculus \cite{FCbook,TFC}.  The tempered fractional derivative
\begin{equation}\label{eq:TFDerivDef}
{\mathbb D}^{d,\lambda}_t f(t)=\lim_{h\to 0} h^{-\alpha}\sum_{j=0}^\infty (-1)^j\binom d  je^{-\lambda jh}f(t-jh)
\end{equation}
has been applied in Finance \cite{carr:geman:madan:yor:2002, carr:geman:madan:yor:2003}, Hydrology \cite{TemperedStable,SettingTime} and Geophysics \cite{Meerschaertsabzikarkumarzeleki} to model processes with {\it semi-heavy tails} \cite{B-N}.  If $\lambda=0$ then \eqref{eq:TFDerivDef} reduces to the Riemann-Liouville fractional derivative \cite{FCbook,oldham,Samko} and \eqref{eq:TFdiffDef} reduces to the fractional difference.}

\begin{defn}\label{defn:artfima general definition}
The discrete time stochastic process $\{X_t\}_{t\in{\mathbb Z}}$ \rev{follows an} {\it autoregressive tempered fractional integrated moving average} time series, denoted by ARTFIMA$(p,d,\lambda,q)$, if
\rev{\begin{equation}\label{eq:artfima general definition1}
Y_{t}=\Delta^{d,\lambda} X_{t}=(I-e^{-\lambda }B)^d X_{t}
\end{equation}
follows the ARMA($p,q$) model
\begin{equation}\label{eq:artfima general definition2}
Y_{t}-\sum_{j=1}^p \phi_{j} Y_{t-j}=Z_{t}+\sum_{i=1}^q \theta_i Z_{t-i}
\end{equation}
}
...
Suppose that $\{X_t\}_{t\in\mathbb{Z}}$ is an {\rm ARTFIMA}$(p,d,\lambda,q)$ time series that satisfies Definition \ref{defn:artfima general definition}
with polynomials $  \Phi(\cdot) $ and $ \Theta(\cdot) $ that satisfy assumption A. Then:
...
$
where $\sum_{j=0}^{\infty}\big|c^{d,\lambda}_{j}\big|<\infty$.  \rev{In particular, $\{X_t\}$ is a stationary time series for all $d\notin {\mathbb Z}$ and all $\lambda>0$.}
...
\begin{rem}
\rev{The well-known ARFIMA$(p,d,q)$ model is a special case of the ARTFIMA$(p,d,\lambda,q)$ model with $\lambda=0$.  Although the ARFIMA model is only stationary for $-.5<d<.5$, Theorem \ref{th2.3} shows that the ARTFIMA model is stationary for any $d\notin {\mathbb Z}$.  This is because tempering avoids the unit root in the ARFIMA model.  For example, the ARFIMA characteristic polynomial $(1-z)$ with $d=1$ has a unit root, but the ARTFIMA characteristic polynomial $(1-e^{-\lambda}z)$ with $d=1$ does not.}
\end{rem}
...
%
\rev{The case $d\in{\mathbb Z}$ is excluded in Definition \ref{defn:artfima general definition} because, in that case, the ARTFIMA$(p,d,\lambda,q)$ model reduces to an ARMA model.  For integer $d>0$, $(1-e^{-\lambda }z)^d$ is a polynomial of degree $d$ that has no zeroes on the unit disk.  Then Definition \ref{defn:artfima general definition} reduces to the ARMA$(p+d,q)$ model $\Phi(B)(I-e^{-\lambda }B)^dX_t=\Theta(B)Z_t$.  For integer $d<0$, the fractional binomial coefficients are undefined, since the recursion $\Gamma(d)=\Gamma(d+1)/d$ involves a divide by zero.  In this case, Definition \ref{defn:artfima general definition} is formally equivalent to the ARMA$(p,q-d)$ model $\Phi(B)X_t=\Theta(B)(I-e^{-\lambda }B)^{-d}Z_t$.  These models are causal and invertible under Assumption A.}
\end{rem}
...
Suppose that $\{X_t\}_{t\in\mathbb{Z}}$ is an {\rm ARTFIMA}$(p,d,\lambda,q)$ time series that satisfies Definition \ref{defn:artfima general definition}, and that \eqref{eq:assumption a} holds.  Then:
\begin{itemize}
...
eventually decays exponentially fast. Following \cite{giraitis}, we say that $X_t$ exhibits {\it semi-long range dependence}.
...
\begin{figure}[ht]\label{FigRev}
\begin{center}
\includegraphics[width=2.5in]{ACFlambda.eps}
\includegraphics[width=2.5in]{ACFd.eps}
\caption{\rev{Theoretical autocorrelation function for the ARTFIMA$(0,d,\lambda,0)$ model.  The left panel shows the ACF for $d = .4$ and $\lambda=0,.001,,05,.1$ (descending).  The right panel shows the} \rev{ACF for $\lambda=.1$ and $d=.3,.6,.9,1.2$ (ascending). }}
\end{center}
\end{figure}

\rev{Figure 1 illustrates the autocorrelation function (ACF) of the ARTFIMA$(0,d,\lambda,0)$ model.  The differencing parameter $d$ acts the same as in the more familiar ARFIMA model.  A higher value of $d$ gives a stronger correlation, so the ACF falls off more slowly with lag.  A higher value of the tempering parameter $\lambda$ makes the ACF fall off more rapidly.  Section \ref{secAppl} shows several more examples of the theoretical ACF, plotted against the periodogram. }
...
\section{Applications}\label{secAppl}
...
\includegraphics[width=5in]{WelshSpecFitNew.eps}
\caption{Spectral density of wall turbulence data (circles), and fitted ARTFIMA spectrum (solid line).}\label{fig3}
\end{center}
\end{figure}
...
\begin{center}
\vskip-50pt
\includegraphics[width=5in]{LnKspecNew.eps}
...
\begin{center}
\vskip-50pt
\includegraphics[width=5in]{y1ARTFIMAspecNew.eps}
\caption{Spectral density of NARCCAP data climate data (circles), fitted ARTFIMA spectrum (solid line) \rev{and untempered ARFIMA spectrum (dotted line)}.}\label{fig5}
\end{center}
\end{figure}

\begin{example}\label{ExNARCCAP}
A North American Regional Climate Change Assessment Program (NARCCAP) climate model was used to generate 29 years of daily maximum temperature data at 16,100 spatial locations in North America.  The data is freely available at {\tt www.narccap.ucar.edu}.  We examine this data at one location, with sample size $n=10,585$.  The time series shows no evidence of a trend at this location, but significant mean variation and heteroscedasticity with respect to the season.  Hence we subtract the seasonal mean, and divide by the seasonal standard deviation.  We fit an ARTFIMA$(0,d,\lambda,0)$ model to the standardized time series with $d=0.933$ and $\lambda=0.300$ using the {\tt artfima} package.  The fitted spectral density and the periodogram are shown in Figure \ref{fig5}.  The spectral density levels off at low frequencies, consistent with the periodogram.  \rev{The spectrum of the untempered ARFIMA model in Figure \ref{fig5} shows a lack of fit at low frequencies. }  As there is no evidence of nonstationarity in the standardized time series, the stationary ARTFIMA model seems \rev{highly} preferable to the stationary increment ARFIMA model,  \rev{which is mis-specified due to its lack of stationarity.}
\end{example}
...
\begin{example}\label{ex:tree-ring}
\rev{Tree-ring time series provide information about past climate
\cite{Fritts,Schofield}.  Hipel and McLeod \cite[p.\ 364]{Hipel1994} fit an ARMA$(2,1)$ model
to a tree-ring time series from a Douglas Fir tree in Colorado.  The time series contains annual indices for $n=858$ consecutive values corresponding to the years 1107--1964.
%NOTE ON CROPPING
%The viewport optional command of the \includegraphics command tells what
%portion of the image file to include. The first two coordinates are the (x,y)
%coordinates in pixels of the lower left corner of the portion of the image
% file you want to include. The second two coordinates are the upper right
% values of (x,y).
%\includegraphics[viewport=5 12 520 345, clip]{tsar-SDEPlots}
%\begin{figure}[h!]
%\caption{Time series plot of the tree-ring time series}
%\begin{center}
%\includegraphics[viewport=5 130 700 330, clip, width=0.7\textwidth]{eaglecol_tsplot.pdf}
%\end{center}
%\label{fig:eaglecol-tsplot}
%\end{figure}
Table \ref{tb:treering} shows that an ARTFIMA$(0,d,\lambda,0)=$ TFD model provides a better fit in terms of AIC and
BIC.  Both ARTFIMA and ARFIMA models were considered, as well as traditional ARMA models.  Both the AIC and BIC selected TFD as the best model.  The fitted parameters of the TFD model are
$\lambda =0.079\ (0.038)$ and  $d=0.529\ (0.0455)$.  The diagnostic plot of the fitted ARTFIMA spectral density function and the observed periodogram (not shown) does not suggest lack-of-fit.  The residual diagnostic plots and portmanteau diagnostic checks also confirm that the model is satisfactory.}

\rev{The rows pAIC and pBIC give the plausibility scores.
The plausibility is defined in a way similar to the relative likelihood
in Fisher likelihood inference.
If ${\rm AIC}_1$, ${\rm AIC}_2$, ${\rm AIC}_3$ and ${\rm AIC}_4$ are four AIC scores
arranged in ascending order, the plausibility for the $i$-th score
is $\exp(-({\rm AIC}_1-{\rm AIC}_i)/2), i=1,2,3,4$, and
similarly for the BIC.}

\begin{table}[h!]
\caption{Best models for Tree-ring Time Series}
\begin{tabular}{ccccc}
  & \text{best} & \text{2nd best} & \text{3rd best} & \text{4th best} \\
\hline
\noalign{\vskip 3pt}
 $\Omega$  & \text{ARTFIMA(0,0,0)} & \text{ARFIMA(2,0,1)} & \text{ARFIMA(1,0,2)} & \text{ARTFIMA(2,0,0)} \\
 \text{AIC} & 8405.03 & 8405.83 & 8405.98 & 8406.39 \\
 \text{pAIC} & 1.000 & 0.668 & 0.621 & 0.507 \\
 $\Omega$  & \text{ARTFIMA(0,0,0)} & \text{ARIMA(1,0,1)} & \text{ARFIMA(0,0,0)} & \text{ARTFIMA(1,0,0)} \\
 \text{BIC} & 8419.29 & 8424.30 & 8424.62 & 8425.94 \\
 \text{pBIC} & 1.000 & 0.082 & 0.070 & 0.036 \\
\end{tabular}
\label{tb:treering}
\end{table}

%\begin{figure}[h!]
%\caption{90\% confidence ellipse}
%\begin{center}
%\includegraphics[width=0.25\textwidth]{eaglecolCI.pdf}
%\end{center}
%\label{fig:eaglecol-ci}
%\end{figure}



%\begin{figure}[h!]
%\caption{Log-log plot of fitted \TFD model and sample spectrum for the tree-ring time series}
%\begin{center}
%\includegraphics[width=0.6\textwidth]{eaglecol_loglog.pdf}
%\end{center}
%\label{fig:eaglecol-loglog}
%\end{figure}

\rev{For forecasting purposes, the BIC is often recommended
\cite[and references therein]{McLeod1993}.
From Table \ref{tb:treering} we see that the ARMA(1,1) provides the second best BIC fit,
 and the FD = ARFIMA$(0,d,0)$ model ranks third.
Although there is little difference in the BIC values between the FD and ARMA(1,1),
Figure \ref{fig:eaglecol-forecast} shows that the forecast performance is very different.
The FD forecast very slowly damps out to the mean level, which is shown by the
dotted line.
The TFD forecast damps out much faster to the mean.
The ARMA$(1,1)$ forecasts are similar to the TFD forecasts.
The last ten years of observed data, shown in Figure \ref{fig:eaglecol-forecast},
suggest there is little long-term predictability, so the TFD or ARMA$(1,1)$
forecasts may be preferable.}
\end{example}

\begin{figure}[h!]
\begin{center}
\includegraphics[width=6.0in]{eaglecolForecast.eps}
\end{center}
\caption{\rev{Last ten years of observed tree-ring data, along with mean (dotted line), TFD forecast (bottom solid line), and FD (top solid line)} \rev{forecast for 100 years.}  }
\label{fig:eaglecol-forecast}
\end{figure}
...
\end{equation*}
\rev{for any fixed $d\notin{\mathbb Z}$} by the fractional binomial formula (e.g., see Hille \cite[p.\ 147]{Hille1959}) it follows that
...
\end{equation}
for $j\geq 0$.
Since $X_t$ satisfies \eqref{eq:artfima general definition1} and \eqref{eq:artfima general definition2}, we can write
...
(a) Use \eqref{eq:artfima general definition1} and \eqref{eq:artfima general definition2} to write $X_t=\psi_X(B)Z_t$ where $\psi_X(z)=(1-e^{-\lambda}z)^{-d}\Theta(z)/\Phi(z)$. Then the general theory of linear filters implies that $X_t$ has spectral density $f_X(\nu)=|\psi_X(e^{-i\nu})|^2f_Z(\nu)$ using the complex absolute value (e.g., see  \cite{BrockwellDavisTSTM}). Since the white noise process $\{Z_t\}$ has spectral density $f_Z(\nu)=(2\pi)^{-1}\sigma^2$, it follows that \eqref{eq:spectraldensity} holds.
...
We would like to thank Mantha Phanikumar, Michigan State University, for access to the water velocity data, and Joe Klewicki, University of Delaware, for the wall turbulence data, as well as helpful discussions.  \rev{We would also like to thank two anonymous referees for their insightful comments.}
...
\bibitem{B-N}
O.E. Barndorff-Nielsen (1998) Processes of normal inverse Gaussian type. {\it Finance Stoch.} {\bf 2}, 41--68.
...
\bibitem{carr:geman:madan:yor:2002}
P. Carr, H. Geman, D.~B. Madan, and M. Yor  (2002)
The fine structure of asset returns: An empirical investigation.
{\it J. Business} {\bf 75}, 303--325.

\bibitem{carr:geman:madan:yor:2003}
P. Carr, H. Geman, D.~B. Madan, and M. Yor (2003)
Stochastic volatility for L\'evy processes.
{\it Math. Finance} {\bf 13}, 345--382.
...
\bibitem{Fritts} H.C.\ Fritts, T.J.\ Blasing, B.P.\ Hayden, and J.E.\ Kutzbach (1971) {Multivariate Techniques for Specifying Tree-growth and Climatic Relationships and for Reconstructing Anomalies in Paleoclimate}. {\it J. Appl. Meteorology} {\bf 10}, 845--864.
...
\bibitem{Hipel1994} K.W.\ Hipel and A.I.\ McLeod (1994) {\it Time Series Modelling of Water Resources and Environmental Systems}.  Elsevier, New York.
...
\bibitem{McLeod1993} A.I.\ McLeod (1993) Parsimony, model adequacy and periodic correlation in forecasting time series. {\it International Statist. Rev.} {\bf 61}, 387--393.

\bibitem{TemperedStable} M.M.\ Meerschaert, Y.\ Zhang, and B.\ Baeumer (2008). Tempered anomalous diffusion in heterogeneous systems. {\it Geophys. Res. Lett.} {\bf 35}, L17403.
...
\bibitem{Molz2004} F.J. Molz, H. Rajaram, and S. Lu (2004) Stochastic fractal-based models in subsurface hydrology: Origins, applications, limitations and
future research questions. {\it Rev. Geophys.} {\bf 42}, RG1002.
...
%
\bibitem{Schofield} M.R.\ Schofield. R.J.\ Barker, A. Gelman , E.R.\ Cook, and K.R.\ Briffa (2016)
{Model-Based Approach to Climate Reconstruction Using Tree-Ring Data}.
{\it J. Amer. Statist. Assoc.} {\bf 111}, 93--106.
%
%\bibitem{Taqqu}
...
\bibitem{SettingTime} Y.\ Zhang and M.M.\ Meerschaert (2011) Gaussian setting time for solute transport in fluvial systems. {\it Water Resour. Res.} {\bf 47}, W08601.
\end{thebibliography}
...
Juan Pablo PÃ©rez Beaupuits 1, Angel OtÃ¡rola 1, Fredrik T. RantakyrÃ¶ 1, Roberto C. Rivera 1, Simon J.
E. Radford 2, Lars-Ã…ke Nyman 1

Wind (spec dens LRD but then proportional to freq for very small freq)
Norton, D.J., â€œMobile Offshore Platform Wind Loads,â€
